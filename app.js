...
(function () {
  ...
  function normalize(raw) {
    ...
    return {
      q: q,
      p: pYes,
      cat: CAT_RU[rawCat] || (rawCat ? rawCat : "Прогноз"),
      vol: fmtVolume(vol),
    };
  }
  ...
  //  Рынки Polymarket — на английском. Переводим бесплатным эндпоинтом
  //  Google Translate (без ключа) и кэшируем в localStorage, чтобы одни и
  //  те же вопросы не переводить повторно. Если перевод не удался —
  //  спокойно оставляем оригинал (английский), игра не ломается.
  // ============================================================
  const CAT_RU = {
    Sports: "Спорт", Soccer: "Футбол", Football: "Футбол", Crypto: "Крипта",
    Politics: "Политика", Economics: "Экономика", Economy: "Экономика",
    Business: "Бизнес", Finance: "Финансы", Science: "Наука", Tech: "Технологии",
    Technology: "Технологии", World: "Мир", Elections: "Выборы", Culture: "Культура",
    "Pop Culture": "Поп-культура", Entertainment: "Развлечения", Climate: "Климат",
    Weather: "Погода", Health: "Здоровье", Geopolitics: "Геополитика", Games: "Игры",
  };

  const TR_CACHE_KEY = "swipe_tr_ru_v1";
  ...
  function saveTrCache(cache) {
    try { localStorage.setItem(TR_CACHE_KEY, JSON.stringify(cache)); } catch (e) {}
  }

  // Похоже ли, что текст уже на русском (есть кириллица) — тогда не переводим.
  ...
  async function translateOne(text) {
    ...
    try {
      ...
      // data[0] — массив сегментов: [[перевод, оригинал, ...], ...]
      const parts = (data && data[0]) || [];
      const out = parts.map((seg) => (seg && seg[0]) || "").join("").trim();
      return out || null;
    } catch (e) {
      clearTimeout(timer);
      return null;
  ...
  // Переводим заголовки колоды (пакетами, чтобы не словить лимиты).
  async function translateDeck(deck) {
    const cache = loadTrCache();
    const todo = [];
    for (const c of deck) {
      if (hasCyrillic(c.q)) { c.qru = c.q; continue; }
      if (cache[c.q]) { c.qru = cache[c.q]; continue; }
      todo.push(c);
    }
    const CHUNK = 8;
    for (let i = 0; i < todo.length; i += CHUNK) {
      ...
      await Promise.all(part.map(async (c) => {
        const t = await translateOne(c.q);
        if (t) { c.qru = t; cache[c.q] = t; }
      }));
    }
    saveTrCache(cache);
  }
  ...
  async function startGame() {
    ...
    if (!deck) {
      ...
      // Ограничиваем колоду и переводим заголовки на русский.
      deck = deck.slice(0, 40);
      el.loadingHint.textContent = "Перевожу заголовки на русский…";
      await translateDeck(deck);
    }
  ...
  function buildCard(data) {
    ...
    card.innerHTML =
      ...
      "</div>" +
      '<div class="card__q">' + escapeHtml(data.qru || data.q) + "</div>" +
